...
### HTTP Headers: POST and Implicit Content Type
...
A Go server based on net/http expects to parse this with

```go
r.ParseForm()
```

Case B: Let the browser send the HTML form data from Js with the fetch API:
...
The Fetch spec mandates this default:

```html
Content-Type: multipart/form-data; boundary=...
```

The Go server needs to parse the data with

```go
r.ParseMultipartForm()
```

Suppose now the browser uses Case B to send the form, but the server assumes Case A. The compiler does not catch anything. ParseForm() silently does nothing. FormValue() sometimes parses multipart (but only if r.Form == nil).
...
Avoid getting data directly with `r.FormValue("username")` and `r.FormValue("password")` as these are not simple getters. They read from stream, invoke parsing, and drain the stream with EOF. This reading needs to be done once at the correct place, preferably inside handler before guards as shown above.
...
One nice thing about Go is that it does not infect everything with goroutines or async/await. initialsdb uses a goroutine just once for graceful shutdown and it is standard code generated by AI in main.go. Handlers are goroutines and they run entire DB transactions, the user does not care about their management.

Still, there are ways to get into trouble, and rate limiting is one area to watch out. See my [previous Go application](https://github.com/aabbtree77/schatzhauser/blob/main/docs/accounts_per_ip.md) where parallel reads in SQLite needed to be artificially serialized with a fake DB write in order to avoid an edge case with IP counter overflow. This is not needed with Postgres.

Rate limiters also use sync.Mutex, but this is not pervasive. It ensures that each request/goroutine updates a hashmap properly atomically, one at a time. Maps in Go are not thread-safe.
