...
- Go compiles to x86 and arm64, so we can do everything on dev and just copy a single file to the container. Compare this to
...
- Go has net/http in stdlib. Coupled with sqlc, this removes paralysis by analysis. Plus a single paradigm for everything: "OO", build system, parallelism. This does not guarantee uniformity of implementations, but still, so rare, so incredible to have.

- DB transactions run inside goroutines, but a user does not care. I used a goroutine once in the entire web app, and it is a standard way to do a graceful shutdown in main.go, generated by AI and tested.

- Go is lower level than Ts, which somehow feels right for backend. One can play with mutexes for concurrent map updates in rate limiters, but this is also avoidable/deferrable to AI/libs. An AI can directly suggest preventing [data races](https://github.com/aabbtree77/schatzhauser/blob/d36c8a98c14679be4e3aa9a4a9cc6d5593476763/internal/handlers/register.go#L91), which may not be in the vocabulary (granularity) of the Node-based app.

The major con of Go is that it is not as succinct as say Python, F#, Nim. It is easier to debug, but it is somewhat not cool. Coolness is important, languages are like motorcycles. GB350 lacks power, but it has style. Go is not there, just like Pascal or Ada, but this is all highly subjective. I chose Go, after all.

Python is more succinct and more beefed up with libs, some pretty unique, but also paralysis-inducing as Js. We get into Django vs Flask vs FastAPI vs Starlette vs Litestar vs PyView... akin to Express vs Hono vs ElysiaJS vs Next.js vs Tanstack Start... Listen to [talkpython.fm](https://talkpython.fm) or [syntax.fm](https://syntax.fm/) for much much more. Async Python is kind of repeating Js, FastAPI Cloud vs Vercel, everything old is new again.

The idea here is to keep React as "modern HTML", strictly for frontend, quite minimally, without React Router. No auth as app's state provider, no context passing and all that jazz. ChatGPT5 initially wrote App.tsx with AbortController API, useRef, useCallback hooks, I then asked it to rewrite it without them.
...
What maybe needs to be done better concretely in this repo is a more succinct workflow from code to "productionization".

On the other hand, a single Makefile "command center" is a disaster, we have seen plenty of these in C++. There are about 5 Makefiles here, so some manual switching/sequencing and verbosity, but once the things go South, at least you know where you are standing.
